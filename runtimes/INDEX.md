...
phase: N/A
updated: 2026-07-09
summary: "Cursor / Claude Code / Codex / OpenCode 运行时对比；CC/OpenCode 模型不固定，须声明 slug。"
---
...
| **adb 烟测** | 当前环境子 agent | 同左 | 同左 | 会话内子 agent | 同左 |
...
| 实施量大、成本敏感 | 在 **当前环境**写代码（CC→mimo、OpenCode→`-m`、Antigravity→`gemini-3.5-flash`）；默认不跨栈 |
| adb 烟测 | **当前环境子 agent** |
| CI 单命令 tick | **Codex** `exec` 或 **Claude** `-p` / **Antigravity** `agy -p`（父 agent 不在该栈时） |
| 方案/评审多视角 | 组合：Cursor 主笔 + mimo / gemini-3.5-flash 实现视角 + 只读 review Task |

## 文档

- [models-and-delegation.md](../models-and-delegation.md) — **必读**：同环境内置 vs 跨环境 CLI  
- [cursor.md](cursor.md) · [claude-code.md](claude-code.md) · [antigravity.md](antigravity.md) · [codex.md](codex.md) · [opencode.md](opencode.md)
